...
import {
  ...
  UserPlus, Clock, Archive, HardDrive, Phone, Trash2,
} from 'lucide-react';
  ...
  return (
    <div className="w-64 h-screen bg-sidebar-background text-sidebar-foreground flex flex-col border-r border-sidebar-border">
      ...
      <div className="p-2">
        ...
        {user?.role === '系統管理員' && navBtn('/recycle-bin', '回收桶', <Trash2 className="w-4 h-4" />)}
        {(user?.role === '管理員' || user?.role === '系統管理員') && navBtn('/contractor', '外包人員管理', <UserPlus className="w-4 h-4" />)}

        {(user?.role === '管理員' || user?.role === '系統管理員') && navBtn('/admin', '系統管理', <Settings className="w-4 h-4" />)}
        {(user?.role === '管理員' || user?.role === '系統管理員') && navBtn('/storage-config', '儲存空間設定', <HardDrive className="w-4 h-4" />)}
      </div>
